...
import coin from '../../images/twkl-coin.png';
...
function MainSection() {
  ...
  useEffect(() => {
    async function fetchTotalFundRaised() {
      if (isConnected) {
        try {
          const web3 = new Web3(window.ethereum);
          const presaleContract = new web3.eth.Contract(PresaleAbi, cAddress);

          // Call the contract method to get the total fund raised
          const totalFundRaised = await presaleContract.methods.totalFundRaised().call();
          setData({ ...data, totalFundRaised: totalFundRaised });
        } catch (error) {
          console.error("Error fetching total fund raised:", error);
        }
      }
    }

    fetchTotalFundRaised();
  }, [isConnected]);
          ...
          <div class="flex h-screen">
            ...
            <div class="w-full bg-gray-100 lg:w-1/2 flex items-center justify-center">
              <div class="max-w-md w-full p-6">
                ...
                <div>
                  <div class="flex m-1 relative items-center justify-start px-6 py-3 overflow-hidden font-medium transition-all bg-green-700 rounded-xl group">
                    ...
                    <span class="absolute bottom-0 left-0 w-full h-full transition-all duration-500 ease-in-out delay-200 bg-green-500-translate-x-full translate-y-full  rounded-2xl group-hover:mb-12 group-hover:translate-x-0"></span>
                    <span class="relative w-full text-center text-white transition-colors duration-200 ease-in-out group-hover:text-white">Tier1 ends in: {timer}</span>
                  </div>
                </div>
                ...
                <div class="space-y-4">
                  ...
                  <div className="relative">
                    <label class="block text-sm font-medium text-gray-700">Pay With</label>
                    <input
                      type="text"
                      value={data.bnb}
                      className="text-black mt-1 p-2 w-full border rounded-md focus:border-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300 transition-colors duration-300"
                      onChange={(e) => {
                    ...
                    <img class="w-7 absolute end-2.5 bottom-1 "
                      src={currentToken.icon}
                      alt="ct" />

                  </div>
                  <div className="relative">
                    <label class="block text-sm font-medium text-gray-700">Get TWKL</label>
                    <input
                      type="text"
                      className="text-black mt-1 p-2 w-full border rounded-md focus:border-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300 transition-colors duration-300"
                      value={data.gart}
                    ...
                    />
                    <img class="w-8 absolute end-2.5 bottom-1 " src={coin} alt="ct" />
                  </div>
                  <div>
